...
// 3) Observer-Optionen
const options = { threshold: 0.1, rootMargin: "0px 0px -150px 0px" };
...
const observer = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    ...
    if (entry.isIntersecting) { // Element betritt den Viewport
      if (isScrollDown) {
    ...
    } else { // Element verlässt den Viewport
      if (!isScrollDown) {
        el.style.transition = "opacity 0.5s ease, transform 0.5s ease";
        el.style.opacity = "0";
...
sections.forEach(el => {
  const isTimeline = el.classList.contains('timeline-item');
  const axis = isTimeline ? 'X' : 'Y';
  const startOffset = isTimeline ? 30 : 30;
  el.style.opacity = "0";
